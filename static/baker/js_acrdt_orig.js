			...
			for (i=0;i<document.links.length;i++){
				oldUrls[i] = document.links[i].href;
					...
					if (oldUrls[i].search(".pdf")!=-1){
						...
						//document.links[i].href="javascript:window.open('acrward.htm?name=" + unescape(pdfname) + "','acrobat','hotkeys=no,resizable=yes,scrollbars=yes,height=350,width=405');window.location='" + currenturl + "';";
...
// END ACROBAT DETECTION



// File Download Counter Script
// by Jarrod Dieppa
// version 1.0 -- 6-30-2004

		beenThrough = false;
		
		function countFileDL(pageName) {
			if (beenThrough == false)  {
				document.counter.src = "http://www.floodmaps.fema.gov/fhmtracker/hitcounter/file_dl_count.asp?fhmpage=" + escape(pageName) +'&rnd='+ Math.random() * 100000000000000000
			}
			setTimeout('window.location.href="'+ pageName +'"',5000);
			beenThrough = true;
			return false;
 		}
		...
		function countFileDL1(pageName) {
			if (beenThrough1 == false)  {
				document.counter.src = "http://www.floodmaps.fema.gov/fhmtracker/hitcounter/file_dl_count.asp?fhmpage=" + escape(pageName) +'&rnd='+ Math.random() * 100000000000000000
			}
			setTimeout('window.location.href="'+ pageName +'"',5000);
			beenThrough1 = true;
